...
export const getToken = async () => {
  console.log('\n\n\n--- UTIL SecureStore token getToken ---\n');
  try {
...
export const setToken = async (token) => {
  console.log('\n\n\n--- UTIL SecureStore token setToken ---\ntoken:', token);

  await SecureStore.setItemAsync('userToken', JSON.stringify(token))
  .then(
    SecureStore.setItemAsync('isLogged', "true").catch((error)=>{ console.log(`setToken isLogged error \n`, error) })
  )
  .catch((error) => { console.log(`setToken error \n`, error) }); 
}
